            ...
            if envia_notificacao:
                ...
                try:
                    email_log = {
                        ...
                        "destinatario": paciente_email,
                        "paciente_nome": data.get('paciente_nome', 'Paciente'),
                        "assunto": "Notificação Bloqueada",
                        "status": "Não Enviado (Desativado)"
                    }
                    supabase.table('historico_emails').insert(email_log).execute()
                except Exception as err:
                    pass
...
# ==========================================
@app.route('/api/tenant/configuracoes', methods=['GET'])
def get_configuracoes():
    ...
    try:
        ...
        if response.data:
            config = response.data[0].get('configuracoes') or {}
            return jsonify({"sucesso": True, "configuracoes": config})
        return jsonify({"sucesso": False, "erro": "Unidade não encontrada"}), 404
    except Exception as e:
        return jsonify({"sucesso": False, "erro": str(e)}), 500
...
def salvar_configuracoes():
    ...
    try:
        supabase.table('unidades').update({"configuracoes": configs}).eq('id', unidade_id).execute()
        return jsonify({"sucesso": True})
    except Exception as e:
        return jsonify({"sucesso": False, "erro": str(e)}), 500
